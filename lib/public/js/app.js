...
FMApp.controller('FileManagerCtr', ['$scope', '$http', '$location',
  function ($scope, $http, $location) {
    ...
    var setCurFiles = function (relPath) {
      $http.get('api' + relPath)
		...
		if (extension == 'jpg' || extension == 'jpeg') {
		    images.push('./api/' + relPath + file.name);
		}
            });
            FM.curFiles = files;
	    FM.images = chunks(images,80);
	    FM.currImages = FM.images[0];
	    FM.pageNumbers = range(0, FM.images.length - 1);
	    // console.log("Pages " + FM.pageNumbers);
	    // console.log("Images len " + FM.images.length);
        })
	...
	// console.log("Hallllooooo " + number);
	FM.currImages = FM.images[number];
    };
